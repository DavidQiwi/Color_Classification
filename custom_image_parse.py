...
def main():
    ...
    parser.add_argument("--custom_image_path", type=str, required=True, help='Path to the image')
    args = parser.parse_args()
    device = 'cuda' if torch.cuda.is_available() else 'cpu'

    #def get_image_path(image_path):
    #custom_image_path = "yell.jpg"
    custom_image_uint8 = torchvision.io.read_image(args.custom_image_path)
    '''print(f"Custom image tensor:\n{custom_image_uint8}\n")
    print(f"Custom image shape: {custom_image_uint8.shape}\n")
    print(f"Custom image dtype: {custom_image_uint8.dtype}")'''

    custom_image = torchvision.io.read_image(args.custom_image_path).type(torch.float32)
